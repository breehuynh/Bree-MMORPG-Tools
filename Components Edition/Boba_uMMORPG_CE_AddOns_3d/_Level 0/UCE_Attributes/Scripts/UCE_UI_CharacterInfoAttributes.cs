...
public partial class UCE_UI_CharacterInfoAttributes : MonoBehaviour
{
    ...
    protected void UpdateAttributes()
    {
        ...
        UIUtils.BalancePrefabs(slotPrefab.gameObject, player.playerAttributes.UCE_Attributes.Count, attributeContent);

        for (int i = 0; i < player.playerAttributes.UCE_Attributes.Count; ++i)
        {
            UCE_UI_AttributeSlot slot = attributeContent.GetChild(i).GetComponent<UCE_UI_AttributeSlot>();
            var attr = player.playerAttributes.UCE_Attributes[i];
            int bonus = player.playerAttributes.UCE_calculateBonusAttribute(attr);

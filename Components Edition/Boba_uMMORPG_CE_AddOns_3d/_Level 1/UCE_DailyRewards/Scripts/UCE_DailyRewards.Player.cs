...
public partial class Player
{
    ...
    private void Start_UCE_DailyRewards()
    {
        if (DailyRewards != null)
        {
            ...
            {
                ((PlayerSkills)skills).skillExperience += ReturnRewards.reward.rewardSkillExperience;
                experience.current += ReturnRewards.reward.rewardExperience;
                gold += ReturnRewards.reward.rewardGold;
                itemMall.coins += ReturnRewards.reward.rewardCoins;
              ...
              rewardHoursPassed >= DailyRewards.LoginIntervalHours)
            {
                ...
                ((PlayerSkills)skills).skillExperience += DailyRewards.rewards[dailyRewardCounter].rewardSkillExperience;
                experience.current += DailyRewards.rewards[dailyRewardCounter].rewardExperience;
                gold += DailyRewards.rewards[dailyRewardCounter].rewardGold;
                itemMall.coins += DailyRewards.rewards[dailyRewardCounter].rewardCoins;
    ...
    public bool UCE_ValidateDailyRewards()
    {
        ...
        valid = (DailyRewards.MinLevel == 0 || level.current >= DailyRewards.MinLevel) ? valid : false;
#if _iMMOQUESTS
        valid = (DailyRewards.requiredQuest == null || UCE_HasCompletedQuest(DailyRewards.requiredQuest.name)) ? valid : false;
#else
		valid = (DailyRewards.requiredQuest == null || quests.HasCompleted(DailyRewards.requiredQuest.name)) ? valid : false;
#endif
    ...
    public bool UCE_ValidateReturnRewards()
    {
        ...
        valid = (ReturnRewards.MinLevel == 0 || level.current >= ReturnRewards.MinLevel) ? valid : false;
#if _iMMOQUESTS
        valid = (ReturnRewards.requiredQuest == null || UCE_HasCompletedQuest(ReturnRewards.requiredQuest.name)) ? valid : false;
#else
		valid = (ReturnRewards.requiredQuest == null || quests.HasCompleted(ReturnRewards.requiredQuest.name)) ? valid : false;
#endif

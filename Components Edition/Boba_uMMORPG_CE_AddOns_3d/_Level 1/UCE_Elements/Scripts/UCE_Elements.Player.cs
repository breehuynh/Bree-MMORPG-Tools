...
public partial class Player
{
    ...
    public override float UCE_CalculateElementalResistance(UCE_ElementTemplate element, bool bCache = true)
    {
        ...
        // -- Bonus: Equipment
        float fEquipmentBonus = (from slot in equipment.slots
                                 where slot.amount > 0
        ...
        foreach (ItemSlot slot in equipment.slots)
        {
            float tmpPointsP = slot.item.setBonusElementPartial(slot, equipment.slots, element);
        ...
        foreach (ItemSlot slot in equipment.slots)
        {
            float tmpPointsC = slot.item.setBonusElementComplete(slot, equipment.slots, element);

...
public partial class UCE_UI_Skills : MonoBehaviour
{
            ...
            for (int i = 0; i < player.skills.skills.Count; ++i)
            {
                if (canShow(i))
                {
                    ...
                    slot.button.interactable = skill.level > 0 &&
                                               !isPassive &&
                                               player.skills.CastCheckSelf(skill); // checks mana, cooldown etc.
                    slot.button.onClick.SetListener(() =>
                    {
                        ((PlayerSkills)player.skills).CmdUse(skillIndex);
                    });
    ...
    private bool canShow(int index)
    {
        ...
        bool valid = ((player.skills.skills[index].category == currentCategory || currentCategory == "") &&
                        (!player.skills.skills[index].unlearnable || (player.skills.skills[index].unlearnable && player.skills.skills[index].level > 0))
                    );

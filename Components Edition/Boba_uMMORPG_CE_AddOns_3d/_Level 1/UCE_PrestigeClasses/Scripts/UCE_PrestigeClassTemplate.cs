        ...
        get
        {
            if (_cache == null)
            {
                ...
                if (entry != null && entry.loadFromAssetBundle)
                    _cache = AssetBundleMagic.LoadBundle(entry.bundleName).LoadAllAssets<UCE_PrestigeClassTemplate>().ToDictionary(x => x.name.GetStableHashCode(), x => x);
                else
                    _cache = Resources.LoadAll<UCE_PrestigeClassTemplate>(folderName).ToDictionary(x => x.name.GetStableHashCode(), x => x);
#else
                _cache = Resources.LoadAll<UCE_PrestigeClassTemplate>(UCE_TemplateConfiguration.singleton.GetTemplatePath(typeof(UCE_PrestigeClassTemplate))).ToDictionary(x => x.name.GetStableHashCode(), x => x);
#endif
            }

...
public partial class Player
{
    ...
    public void UCE_RecoverOverTime(UCE_Area_Sanctuary sanctuary)
    {
            ...
            )
        {
            ...
            if (gainHealth > 0 && health.current < health.max)
            {
                health.current += gainHealth;
                UCE_TargetAddMessage(sanctuary.MSG_HEALTH + gainHealth.ToString());
            }

            if (gainMana > 0 && mana.current < mana.max)
            {
                mana.current += gainMana;
            ...
            if (gainExp > 0)
            {
                experience.current += gainExp;
                UCE_TargetAddMessage(sanctuary.MSG_EXP + gainExp.ToString());
            }

            if (gainSkillExp > 0)
            {
                ((PlayerSkills)skills).skillExperience += gainSkillExp;

...
public partial class Player
{
    ...
    {
        return guild.guild.members.Length < guildCapacity &&
               guild.guild.CanInvite(requesterName, targetName);
    }
    ...
    public void UCE_UpgradeGuild()
    {
        if (UCE_CanUpgradeGuild())
        {
            ...
            {
                inventory.Add(new Item(guildUpgrades.rewardItem[guildLevel].item), guildUpgrades.rewardItem[guildLevel].amount);
            }
    ...
    public bool UCE_CanAddRewardItem()
    {
        return
                ...
                (
                guildUpgrades.rewardItem.Length >= guildLevel &&
                inventory.CanAdd(new Item(guildUpgrades.rewardItem[guildLevel].item), guildUpgrades.rewardItem[guildLevel].amount)
                )
                );
    }
    ...
    public void UCE_BroadCastPopupToOnlineGuildMembers(string message)
    {
        ...
        {
            if (guild.InGuild() && player.guild.InGuild() && guild.name == player.guild.name)
            {

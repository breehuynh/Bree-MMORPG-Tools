...
public partial class UCE_UISkillbarMobile : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (player.skillbar.slots.Length < 8)
        {
        ...
        if (!initialized)
        {
            ...
            for (int i = itemStart + 2; i < player.skillbar.slots.Length; ++i)
            {
        ...
        // Attack Button
        if (player.skillbar.slots[0].reference == "")
            player.skillbar.slots[0].reference = player.skills.skills[0].name;
    }
}

...
public partial struct Skill
{
    ...
    public bool UCE_CheckSkillConditions(Entity caster)
    {
        ...
        if (data.skillConditions.healthThreshold == Monster.ParentThreshold.Above && (caster.health.current < caster.health.max * data.skillConditions.casterHealth))
            bValid = false;

        if (data.skillConditions.healthThreshold == Monster.ParentThreshold.Below && (caster.health.current > caster.health.max * data.skillConditions.casterHealth))
            bValid = false;

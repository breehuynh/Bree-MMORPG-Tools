...
public partial class Player
{
    ...
    public override void UCE_CleanupStatusBuffs(float successChance = 1f, float modifier = 0f, int limit = 0)
    {
        ...
        {
            if (!skills.buffs[i].data.disadvantageous && skills.buffs[i].data != offenderBuff && skills.buffs[i].data != murdererBuff && UCE_CheckChance(successChance, modifier))
            {
    ...
    // @Server
    // -----------------------------------------------------------------------------------
    public void UCE_removeItems(Player player, UCE_ItemRequirement[] items, bool removeAll = false)
    {
    ...
    // -----------------------------------------------------------------------------------
    public int UCE_inventorySlotCount(Player player)
    {
        int amnt = 0;

...
public partial class Entity
{
    ...
    {
        if (fChance <= 0 || source.skills.skills.Count <= 0 || !isAlive || !UCE_CanAttack(source)) return;
    ...
    public virtual void UCE_CleanupStatusBuffs(float successChance = 1f, float modifier = 0f, int limit = 0)
    {
        ...
        for (int i = 0; i < skills.buffs.Count; ++i)
        {
            if (!skills.buffs[i].data.disadvantageous && UCE_CheckChance(successChance, modifier))
            {
                if (!skills.buffs[i].data.cannotRemove)
                {
                    skills.buffs.RemoveAt(i);
    ...
    // UCE_checkHasItems
    // -----------------------------------------------------------------------------------
    public bool UCE_checkHasItems(Player player, UCE_ItemRequirement[] items, bool requiresAll = false)
    {
        if (items == null || items.Length == 0) return true;

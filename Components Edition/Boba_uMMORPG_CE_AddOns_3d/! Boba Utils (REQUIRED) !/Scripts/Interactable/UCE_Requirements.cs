...
public partial class UCE_Requirements
{
    ...
    public virtual bool checkRequirements(Player player)
    {
        ...
        valid = (requiredItems.Length == 0 || player.UCE_checkHasItems(player, requiredItems)) ? valid : false;
#if _iMMOPRESTIGECLASSES
...
#if _iMMOGUILDUPGRADES
		if (minGuildLevel > 0)
			valid = player.guild.InGuild() && player.guildLevel >= minGuildLevel ? valid : false;
#endif
    ...
    protected bool checkAttributes(Player player)
    {
        ...
        foreach (UCE_AttributeRequirement requirement in requiredAttributes)
        {
            if (requirement.template != null &&
                (requirement.minValue <= 0 || player.playerAttributes.UCE_Attributes.FirstOrDefault(x => x.template == requirement.template).points >= requirement.minValue) &&
                (requirement.maxValue <= 0 || player.playerAttributes.UCE_Attributes.FirstOrDefault(x => x.template == requirement.template).points <= requirement.maxValue)
                )
            {

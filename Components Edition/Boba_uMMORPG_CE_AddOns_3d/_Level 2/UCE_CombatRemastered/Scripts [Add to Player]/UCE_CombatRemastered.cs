...
public class UCE_CombatRemastered : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (player != null)
        {
            if (player.equipment.slots[0].amount == 0 && player.equipment.slots[4].amount == 0)
            {
                SetupCombatType();
                return;
            }
            ...
            if (player.equipment.slots[0].amount > 0)
                AutoRemoveEquipment(player.equipment.slots[0]);
            else
                AutoRemoveEquipment(player.equipment.slots[4]);
        }
    }
    ...
    private void Update()
    {
        if (player != null)
            if (player.isClient && player.state == "CASTING")
                if (player.skills.skills[0].IsCasting())
                    animator.SetBool(player.skills.skills[player.autoAttack].name, player.skills.skills[0].IsCasting());
    }
    ...
    public void AutoRemoveEquipment(ItemSlot slot)
    {
        if (waitTimer <= Time.time)
        {
            ...
            mainSlot = player.equipment.slots[0]; offSlot = player.equipment.slots[4]; mainItem = mainSlot.item; offItem = offSlot.item;
            ...
            switch (changedScriptItem.equipType)
            {
                case EquipmentItem.EquipType.Slash1Hand:
                    if (offSlot.amount > 0 && player.inventory.SlotsFree() >= offSlot.amount)
                    {
                        ...
                        {
                            player.CmdAutoRemoveEquipment(player.equipment.slots.IndexOf(offSlot));
                            player.inventory.Remove(mainItem, 1);
                        }
                    }
                ...
                case EquipmentItem.EquipType.RangedBow:
                    if ((mainSlot.amount > 0 && player.inventory.SlotsFree() >= mainSlot.amount) && offSlot.amount > 0)
                    {
                        player.CmdAutoRemoveEquipment(player.equipment.slots.IndexOf(mainSlot));
                        player.inventory.Remove(offItem, 1);
                    }
                ...
                case EquipmentItem.EquipType.Shield:
                    if (mainSlot.amount > 0 && player.inventory.SlotsFree() >= mainSlot.amount)
                    {
            ...
            if (mainSlot.amount > 0 && offSlot.amount > 0)
                if (((EquipmentItem)player.equipment.slots[4].item.data).equipType == EquipmentItem.EquipType.Shield && ((EquipmentItem)player.equipment.slots[0].item.data).equipType != EquipmentItem.EquipType.RangedThrown)
                    currentCombatType = CombatType.Shield;

            if (mainSlot.amount > 0 && offSlot.amount > 0)
                if (((EquipmentItem)player.equipment.slots[4].item.data).equipType == EquipmentItem.EquipType.Dual)
                    currentCombatType = CombatType.Dual;

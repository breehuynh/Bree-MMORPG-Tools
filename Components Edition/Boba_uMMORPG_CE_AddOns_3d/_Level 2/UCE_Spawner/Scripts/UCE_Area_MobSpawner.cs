        ...
        foreach (UCE_MobSpawnableGameObject spawnableEntity in spawnableGameObjects)
        {
            if (spawnableEntity.probability > 0 && UnityEngine.Random.value <= spawnableEntity.probability)
            {
                ...
                if (go.GetComponent<Entity>() != null)
                {
                    ...
                    {
                        go.GetComponent<Entity>().level.current = (int)(go.GetComponent<Monster>().level.current + spawnableEntity.levelAdjustment) / 2;
                    }
                    ...
                    {
                        go.GetComponent<Entity>().level.current = (int)(go.GetComponent<Entity>().level.current + currentPlayerLevel + spawnableEntity.levelAdjustment) / 2;
                    }
                }
            }
        }

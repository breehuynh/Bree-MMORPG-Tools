...
public partial class Player
{
    ...
    public bool UCE_Crafting_CraftValidation(UCE_Tmpl_Recipe recipe, int amount, bool boost, bool craftable = true)
    {
        ...
        {
            if (inventory.Count(new Item(ingredient.item)) < (ingredient.amount * amount))
                valid = false;
        }

        // ----- check mana
        if (mana.current < recipe.manaCost)
            valid = false;
            ...
            if (UCE_Crafting_CraftValidation(UCE_myRecipe, amount, boost))
            {
                ...
                for (int i = 1; i <= amount; i++)
                    ...
                    if (UnityEngine.Random.value <= successProbability)
                    {
                        ...
                        experience.current += UCE_myRecipe.experience;
                        ((PlayerSkills)skills).skillExperience += UCE_myRecipe.skillExp;
                        ...
                        if (UnityEngine.Random.value <= criticalProbability)
                        {
                            ...
                            if (UCE_myRecipe.criticalResult.Length > 0)
                            {
                                foreach (UCE_CraftingRecipeIngredient result in UCE_myRecipe.criticalResult)
                                    inventory.Add(new Item(result.item), result.amount);
                            }
                            else
                            {
                ...
                foreach (UCE_CraftingRecipeIngredient ingredient in UCE_myRecipe.ingredients)
                {
                    ...
                    inventory.Remove(new Item(ingredient.item), (ingredient.amount * amount));
                }

                // --------------------------------------------------------------------------- Deplete other Costs (mana etc.)

                mana.current -= UCE_myRecipe.manaCost;
                ...
                if (boost)
                {
                    ...
                    {
                        if ((!tool.equippedItem && inventory.Count(new Item(tool.requiredItem)) >= amount) || (tool.equippedItem && UCE_checkHasEquipment(tool.requiredItem)))
                        {
                            if (UnityEngine.Random.value <= tool.toolDestroyChance)
                            {
                                if (tool.equippedItem)
                                    ...
                                    inventory.Remove(new Item(tool.requiredItem), amount);
    ...
    public bool UCE_Crafting_CanBoost(UCE_Tmpl_Recipe recipe, int amount)
    {
        ...
        {
            if (
                (!tool.equippedItem && inventory.Count(new Item(tool.requiredItem)) >= amount) ||

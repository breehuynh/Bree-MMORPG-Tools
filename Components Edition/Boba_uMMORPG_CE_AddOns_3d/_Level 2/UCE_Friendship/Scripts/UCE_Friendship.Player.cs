...
public partial class Player
{
    ...
    {
        if (string.IsNullOrWhiteSpace(name) || !guild.InGuild()) return;

        Player plyr = Player.onlinePlayers[name];

        if (plyr && !plyr.guild.InGuild() && guild.guild.CanInvite(this.name, name) && NetworkTime.time >= nextRiskyActionTime)
        {
            plyr.guild.inviteFrom = this.name;
            nextRiskyActionTime = Time.time + guild.inviteWaitSeconds;
        }
    }

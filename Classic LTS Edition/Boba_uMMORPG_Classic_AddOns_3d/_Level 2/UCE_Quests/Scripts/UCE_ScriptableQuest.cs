        ...
        get
        {
            if (_cache == null)
            {
                ...
                if (entry != null && entry.loadFromAssetBundle)
                    _cache = AssetBundleMagic.LoadBundle(entry.bundleName).LoadAllAssets<UCE_ScriptableQuest>().ToDictionary(x => x.name.GetStableHashCode(), x => x);
                else
                    _cache = Resources.LoadAll<UCE_ScriptableQuest>(folderName).ToDictionary(x => x.name.GetStableHashCode(), x => x);
#else
                _cache = Resources.LoadAll<UCE_ScriptableQuest>(UCE_TemplateConfiguration.singleton.GetTemplatePath(typeof(UCE_ScriptableQuest))).ToDictionary(x => x.name.GetStableHashCode(), x => x);
#endif
            }

        ...
        get
        {
            if (_cache == null)
            {
                ...
                if (entry != null && entry.loadFromAssetBundle)
                    _cache = AssetBundleMagic.LoadBundle(entry.bundleName).LoadAllAssets<UCE_Tmpl_AdminCommand>().ToDictionary(x => x.name.GetStableHashCode(), x => x);
                else
                    _cache = Resources.LoadAll<UCE_Tmpl_AdminCommand>(folderName).ToDictionary(x => x.name.GetStableHashCode(), x => x);
#else
                _cache = Resources.LoadAll<UCE_Tmpl_AdminCommand>(UCE_TemplateConfiguration.singleton.GetTemplatePath(typeof(UCE_Tmpl_AdminCommand))).ToDictionary(x => x.name.GetStableHashCode(), x => x);
#endif
            }

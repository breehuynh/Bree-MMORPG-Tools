    ...
    private void OnSubmit_GlobalChat(string text)
    {
        ...
        if (text.StartsWith(global.command))
        {
            ...
            else
            {
                ...
                {
                    UIChat.singleton.AddMessage(new ChatMessage("", info.identifierIn, errorMessage, "", global.textPrefab));
                }
            }
        }
    }
        ...
        if (player != null)
        {
            ...
            if (player.level >= globalChatLevel)
            {
                ...
                if (Time.time >= nextChatTime)
                {
                    ...
                    if (globalChatItem == null || player.InventoryCount(new Item(globalChatItem)) > 0)
                    {

        ...
        get
        {
            if (_cache == null)
            {
                ...
                if (entry != null && entry.loadFromAssetBundle)
                    _cache = AssetBundleMagic.LoadBundle(entry.bundleName).LoadAllAssets<UCE_ElementTemplate>().ToDictionary(x => x.name.GetStableHashCode(), x => x);
                else
                    _cache = Resources.LoadAll<UCE_ElementTemplate>(folderName).ToDictionary(x => x.name.GetStableHashCode(), x => x);
#else
                _cache = Resources.LoadAll<UCE_ElementTemplate>(UCE_TemplateConfiguration.singleton.GetTemplatePath(typeof(UCE_ElementTemplate))).ToDictionary(x => x.name.GetStableHashCode(), x => x);
#endif
            }

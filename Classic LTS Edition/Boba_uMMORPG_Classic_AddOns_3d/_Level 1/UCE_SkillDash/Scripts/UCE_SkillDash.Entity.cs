...
public partial class Entity
{
    ...
    public void StartDash(Vector3 newPos, float time)
    {
        ...
        {
            Debug.DrawLine(agent.transform.position, hit.position, Color.white, 3f);
            int layerMask = 1 << 8;
            layerMask = ~layerMask;
            RaycastHit hit2;
            if (Physics.Linecast(agent.transform.position + (Vector3.up * 0.5f), hit.position + (Vector3.up * 0.5f), out hit2, layerMask))
            {
                Debug.DrawLine(agent.transform.position + (Vector3.up * 0.5f), (hit2.point + (Vector3.up * 0.5f)) + (agent.transform.position - hit2.point).normalized, Color.red, 3f);
                if (NavMesh.SamplePosition((hit2.point - (Vector3.up * 0.5f)) + (agent.transform.position - hit2.point).normalized, out hit, 1, NavMesh.AllAreas))
                {
                    Debug.DrawLine(agent.transform.position, hit.position, Color.green, 3f);
            ...
            dashMaxTime = time;
            isDash = true;
            dashOldPosition = agent.transform.position;
    ...
    {
        if (isServer && agent.velocity.magnitude != 0 && agent.velocity.normalized != Vector3.zero)
        {
